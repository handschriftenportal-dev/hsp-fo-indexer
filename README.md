...
* *common.restart-interval:* interval (in ms), that is uses for restarting the service after a serious error
* *kafka.bootstrap-servers:* list of kafka brokers
* *kafka.topic*: the name of the kafka topic
* *solr.host*: solr host, where the data should be written to.
* *solr.core*: name of the solr core (usually `hsp`)
* *solr.schema-version*: the schema version (can be found in version.yaml) - there's no need the change, as long there are no changes to the mapping algorithm. Increasing the schema version will cause a removal of the whole Solr dataset and re-indexing it.
