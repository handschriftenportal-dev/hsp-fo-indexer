...
public enum ResultMapper {
   ...
   */
  private static Optional<GNDEntity> getFirstEntity(final Object obj) {
    if (Objects.nonNull(obj) && obj instanceof List && !((List<?>) obj).isEmpty() && ((List<?>) obj).get(0) instanceof GNDEntity) {
      return Optional.of(((List<GNDEntity>) obj).get(0));
    } else {
  ...
  private static List<String> mapGNDEntity(final Object objectToMap, final Function<GNDEntity, List<String>> map) {
    final Optional<GNDEntity> gndEntity = getFirstEntity(objectToMap);
