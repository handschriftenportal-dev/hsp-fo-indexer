...
/**
 * The {@code process} implementation of this class fetches a corporate authority file by using given ids
 * and returns all of its values as {@code List} if an authority file was found. If the given id is blank, no request is performed.
 * In this case, an empty list is returned.
 */
public class CorporatePostProcessor extends PostProcessor implements IAttributePostProcessor {
  /**
   ...
   * @param values       the attributes mapped value (the ids)
  ...
  public List<String> process(final XMLSource source, final List<String> values, final ResultMapper resultMapper, final IContentResolver... resolver) throws ContentResolverException {
    return process(source, values, resultMapper, GNDEntityType.CORPORATE, resolver);
  }
}
