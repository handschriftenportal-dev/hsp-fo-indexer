...
class HspObjectGenericRepositoryServiceTest extends BaseRepositoryTest {

  private static GenericRepositoryServiceImpl<HspBaseTestClass> repository;
  /* necessary when checking if first repository really only works on the given type */
  private static GenericRepositoryServiceImpl<HspBaseTestClass> secondRepository;
  private final Map<HspObjectType, GenericRepositoryServiceImpl<HspBaseTestClass>> repositories = new EnumMap<>(HspObjectType.class);

  @BeforeEach
  void beforeEach() {
    Stream.of(HspObjectType.values())
        .forEach(type -> repositories.put(type, new GenericRepositoryServiceImpl<>(persistenceService, type)));
