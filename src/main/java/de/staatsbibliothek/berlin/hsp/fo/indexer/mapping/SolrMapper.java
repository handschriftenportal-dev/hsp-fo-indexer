...
public class SolrMapper {
  ...
  public Optional<HspObjectGroup> mapHspObjectGroup(@Nullable final byte[] kodContent, List<byte[]> descriptionContents) {
    ...
    final Boolean digitizedIIIFObject = CollectionUtils.emptyIfNull(digitizeds).stream()
            .anyMatch(digitized -> StringUtils.isNotBlank(digitized.getManifestURLSearch()));
    ...
    FacetHelper.enrichFacets(hspObjectGroup);

    return Optional.of(hspObjectGroup);
  }

  public Optional<HspObject> mapHspObject(final byte[] tei) {
  ...
  public Optional<HspDescription> mapHspDescription(final byte[] tei, final HspObject obj, final Boolean digitizedObject, final Boolean digitizedIiifObject) {
  ...
  public List<HspDigitized> mapHspDigitizeds(final byte[] tei, final String groupId, final String repositoryId) {
