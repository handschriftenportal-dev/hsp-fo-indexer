 ...
 */
public class CorporatePostProcessor implements IAttributePostProcessor {
  static final Logger logger = LoggerFactory.getLogger(CorporatePostProcessor.class);
  ...
  public List<String> process(final XMLSource source, final List<String> values, final ResultMapper resultMapper, final IContentResolver... resolver) throws ContentResolverException {
    ...
    if (CollectionUtils.isNotEmpty(values) && StringUtils.isNotBlank(values.get(0))) {
      entities = PostProcessingHelper.getNormdatumList(values.get(0), GNDEntityType.CORPORATE, resolver);
