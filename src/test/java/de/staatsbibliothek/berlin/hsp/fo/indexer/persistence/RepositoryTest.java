...
class RepositoryTest extends BaseSolrTest {
  ...
  private final SolrRepositoryImpl<HspBaseTestClass> persistenceService;
  ...
  void whenRemoveByQueryIsCalledWithMatchingQuery_ThenAllMatchingObjectsShouldBeRemoved() throws Exception {
    ...
    persistenceService.remove(String.format("type-search:\"%s\"", HspObjectType.OBJECT.getValue()[0]));
    loaded = persistenceService.find("*:*");
    assertThat(loaded, hasSize(1));
  ...
  void whenFindByQueryIsCalledWithNoneMatchingQuery_ThenNoEntitiesShouldBeReturned() throws Exception {
    ...
    Collection<HspBaseTestClass> loaded = persistenceService.find(String.format("%s:%s AND %s:\"%s\"", Constants.FIELD_NAME_ID, "testid3", Constants.FIELD_NAME_TYPE, Arrays.toString(HspObjectType.OBJECT.getValue())));
    assertThat(loaded, hasSize(0));
  }
}
