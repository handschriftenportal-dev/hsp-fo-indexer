...
public class SchemaVersionServiceImpl implements SchemaVersionService {
   ...
   * @return true if {@link version} is higher or equal to {@code versionToCompare}, false otherwise
  ...
  public static boolean isUpToDate(final String version, final String versionToCompare) throws SchemaUpdateException {
    ...
    if (comparisonResult > 0) {
      throw new SchemaUpdateException(String.format("The current schema version is higher than the expected one. Expected version is: %s Current version is: %s", versionToCompare, version), null);
