...
public abstract class PostProcessor {
  ...
  protected List<String> process(final XMLSource source, final List<String> values, final ResultMapper resultMapper, final GNDEntityType type, final IContentResolver... resolver) throws ContentResolverException {
    ...
    if (CollectionUtils.isNotEmpty(values) && StringUtils.isNotBlank(values.getFirst()) && AUTHORITY_FILE_PATTERN.matcher(values.getFirst()).matches()) {
      entities = PostProcessingHelper.getAuthorityFiles(values.getFirst(), type, resolver);

      if (entities.size() > 1 && log.isInfoEnabled()) {
        log.info("Multiple authority files for {} were found. Only the first will be used", values.getFirst());
      }
      if(entities.isEmpty() && log.isInfoEnabled()) {
        log.warn("No authority file found for {}", values.getFirst());
      }
    }
